...
class Pagination {
	...
	public function render() {
		...
		if ($num_pages > 1) {
			...
			for ($i = $start; $i <= $end; $i++) {
				if ($page == $i) {
					$output .= '<li class="page-item active"><a href="javascript:;" class="page-link">' . $i . '</a></li>';
				} else {
					if ($i === 1) {
		...
		if ($page < $num_pages) {
			$output .= '<li class="page-item"><a href="' . str_replace('{page}', $page + 1, $this->url) . '" class="page-link" aria-label="' . $this->text_next . '"><span aria-hidden="true">' . $this->text_next . '</span></a></li>';
			$output .= '<li class="page-item"><a href="' . str_replace('{page}', $num_pages, $this->url) . '" class="page-link" aria-label="' . $this->text_last . '"><span aria-hidden="true">' . $this->text_last . '</span></a></li>';
		}
