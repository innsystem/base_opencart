...
class ControllerUserUserPermission extends Controller {
	...
	protected function getList() {
		...
		if ($user_info) {
			$data['user_firstname'] = $user_info['firstname'];
			$data['user_lastname'] = $user_info['lastname'];
			$data['user_username']  = $user_info['username'];
			$data['user_user_group'] = $user_info['user_group'];
		} 
	...
	protected function getForm() {
		...
		if (isset($this->request->get['user_group_id']) && $this->request->server['REQUEST_METHOD'] != 'POST') {
			if($user_group_info['name'] == "Desenvolvedor"){
				if($user_info['user_group'] != "Desenvolvedor"){
					$this->response->redirect($this->url->link('user/user_permission', 'user_token=' . $this->session->data['user_token'] . $url, true));
				}
			}
		}
