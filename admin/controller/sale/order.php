...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['sticker'] = $this->url->link('sale/order/sticker', 'user_token=' . $this->session->data['user_token'], true);
		...
		foreach ($results as $result) {
			$getOrderStatus = $this->model_sale_order->getOrderStatusName($result['order_status']);
			if($getOrderStatus){
			foreach ($getOrderStatus as $getOrderStatuses){
			$name_status = $getOrderStatuses['color_type'];
			}
			}else{
			$name_status = 'badge-dark';
			}

				$data['orders'][] = array(
					'order_id'      => $result['order_id'],
					'customer'      => $result['customer'],
					'order_status' => $result['order_status'] ? '<span class="badge '.$name_status.'">'.$result['order_status'].'</span>' : '<span class="badge '.$name_status.'">'.$this->language->get('text_missing').'</span>',
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['sticker'] = $this->url->link('sale/order/sticker', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			...
			$replace = array(
				...
				'number' => $order_info['payment_custom_field'][2],
			...
			$replace = array(
				...
				'number' => isset($order_info['shipping_custom_field'][2]) ? $order_info['shipping_custom_field'][2] : $order_info['payment_custom_field'][2],
	...
	public function sticker() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
					$format = '{firstname} {lastname} <br/> CPF: ' . $order_info['custom_field']['1'] . "\n" . 'CEP: {postcode} <br/> {address_1}, {numero} <br/> {address_2}' . ' - ' . '{city} / {zone_code} - {country}';
			...
			if(isset($data['shipping_custom_fields'][0]['value'])){
				$numero = $data['shipping_custom_fields'][0]['value'];
			}else{
				$numero = $data['payment_custom_fields'][0]['value'];
			}
			...
			$replace = array(
				...
				'zone' => ($order_info['shipping_zone'] ? $order_info['shipping_zone'] : $order_info['payment_zone']),
				'zone_code' => ($order_info['shipping_zone_code'] ? $order_info['shipping_zone_code'] : $order_info['payment_zone_code']),
				'country' => ($order_info['shipping_country'] ? $order_info['shipping_country'] : $order_info['payment_country']),
			...
			$shipping_address = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
				...
				$data['orders'][] = array(
					...
					'correios'       	=> ($order_info['shipping_code'] ? $order_info['shipping_code'] : $order_info['payment_code']),
					...
					'cep' 				=> ($order_info['shipping_postcode'] ? $order_info['shipping_postcode'] : $order_info['payment_postcode']),
		...
		$this->response->setOutput($this->load->view('sale/etiqueta_correios', $data));
	}
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($order_info['payment_address_format']) {
					...
					$format = '<b>{firstname} {lastname}</b><br><b>CEP:{postcode}</b><br>{address_1} {address_2}, {company}<br> {city} - {zone} - {country}';
				}
	...
	public function setstatus() {
		...
		if(isset($this->request->get['order_id'])){
			$requestpart = $this->request->get['order_id'];
			$typeStatus = $this->request->get['type_status'];

			$column_name = 'order_status_id';
			$order_id = $requestpart;				
			$result = $this->model_sale_order->getOrder($order_id);
			// $this->model_sale_order->setStatus($order_id, $column_name, $typeStatus);
			$comment = '';
			$this->model_sale_order->addOrderHistory($order_id, $typeStatus, $comment, true);
			$result = $this->model_sale_order->getOrder($order_id);

			$this->load->model('localisation/order_status');
			$getorderStatus = $this->model_sale_order->getOrderStatus();

			foreach ($getorderStatus as $orderStatus) {
				if($result[$column_name] == $orderStatus['order_status_id']){
					$output = "<span class='badge ".$orderStatus['color_type']."'>".$orderStatus['name']."</span>";
				}
			}
