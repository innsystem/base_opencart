...
class ControllerProductProduct extends Controller
{
	...
	public function index()
	{
		...
		if ($product_info) {
			...
			$imgOld = $imgProd;
			$imgNew = str_replace(" ", "%20", $imgOld);
			$this->document->addFBMeta('og:image:url', $imgNew);
			...
			if ($this->config->get('total_reward_status') == 1) {
			...
			$pasta = '_galerias/avatares/';
			$arquivos = glob("$pasta{*.png}", GLOB_BRACE);
			$data['totalAvatares'] = count($arquivos);
	...
	public function review()
	{
		...
		$results = $this->model_catalog_review->getReviewsByProductId($this->request->get['product_id'], ($page - 1) * 4, 4);

		foreach ($results as $result) {
			$data['reviews'][] = array(
				'author'     => $result['author'],
				'text'       => nl2br(mb_strimwidth($result['text'], 0, 155, "...")),
				'rating'     => (int)$result['rating'],
	...
	public function write()
	{
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ((utf8_strlen($this->request->post['avatar']) < 0) || (utf8_strlen($this->request->post['avatar']) > 1000)) {
				$json['error'] = 'É necessário escolher um avatar.';
			}
