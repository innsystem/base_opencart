...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		if ($order_info['payment_address_format']) {
			...
			$format = '<b>{firstname} {lastname}</b><br><b>CEP:{postcode}</b><br>{address_1}, N° {number} {address_2}, {company}<br> {city} - {zone} - {country}';
		}
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			$data['order_id'] = $order_info['order_id'];
			$data['name_cliente'] = $order_info['firstname'] . ' ' . $order_info['lastname'];
			...
			foreach ($order_products as $order_product) {
				...
				$data['order_value_total'] = html_entity_decode($this->currency->format($order_product['total'] + ($this->config->get('config_tax') ? ($order_product['tax'] * $order_product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8');
